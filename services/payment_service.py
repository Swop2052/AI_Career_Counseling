...
class PaymentService:
    ...
    def create_payment_order(
        ...
        try:
            with conn:
                cursor = conn.cursor()
                # Clean up any stale unfulfilled 'CREATED' orders for this user and plan to prevent accumulating duplicate abandoned records
                cursor.execute("""
                    UPDATE payments
                    SET status = 'CANCELLED', updated_at = %s
                    WHERE user_id = %s AND plan_id = %s AND status = 'CREATED'
                """, (now_str, user_id, plan_id))
                ...
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'razorpay', %s, 'CREATED', %s, %s)
                """, (
    ...
    def verify_and_process_payment(
        ...
        try:
            ...
            if not self.provider.verify_signature(sig_params):
                with conn:
                    conn.execute("""
                        UPDATE payments SET status = 'FAILED', razorpay_payment_id = %s, razorpay_signature = %s, updated_at = %s
                        WHERE id = %s
            ...
            with conn:
                ...
                cursor.execute("""
                    UPDATE payments
                    SET status = 'SUCCESS', razorpay_payment_id = %s, razorpay_signature = %s, updated_at = %s
                ...
                if cursor.rowcount == 0:
                    ...
                    cursor.execute("SELECT status FROM payments WHERE id = %s", (payment_row['id'],))
    ...
    def mark_payment_cancelled(self, user_id: str, razorpay_order_id: str) -> bool:
        """Mark an unfulfilled CREATED payment as CANCELLED when user dismisses or cancels checkout."""
        now_str = datetime.now().isoformat()
        conn = get_db_connection()
        try:
            with conn:
                ...
                cursor.execute("""
                    UPDATE payments
                    SET status = 'CANCELLED', updated_at = %s
                    WHERE user_id = %s AND razorpay_order_id = %s AND status = 'CREATED'
                """, (now_str, user_id, razorpay_order_id))
        ...
        try:
            with conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, 0.0, %s, %s, %s, %s, 'referral_zero', %s, %s, 'SUCCESS', %s, %s)
                """, (
        ...
        if event_type in ('order.paid', 'payment.captured'):
            ...
            try:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""
                        UPDATE payments SET status = 'SUCCESS', razorpay_payment_id = %s, updated_at = %s
                        WHERE id = %s AND status = 'CREATED'
            ...
            if order_id:
                ...
                try:
                    with conn:
                        conn.execute("""
                            UPDATE payments SET status = 'FAILED', razorpay_payment_id = %s, updated_at = %s
