...
class TestAssessmentHistoryRetrieval(unittest.TestCase):
    def setUp(self):
        ...
        with conn:
            conn.execute("""
                ...
                VALUES (%s, %s, 'dummy_hash', 'USER', 1)
            """, (self.uid, self.email))
            conn.execute("""
                INSERT INTO user_profiles (id, user_id, full_name, education_level)
                VALUES (%s, %s, 'History Test Student', 'Grade 12')
            ...
            conn.execute("""
                INSERT INTO user_profiles (id, user_id, full_name, education_level)
                VALUES (%s, %s, 'Other Test Student', 'Grade 11')
            """, (f"prof_{self.other_uid}", self.other_uid))

            conn.execute("""
                INSERT INTO users (id, email, password_hash, role, is_active)
                VALUES (%s, %s, 'dummy_hash', 'DEVELOPER', 1)
    ...
    def tearDown(self):
        conn = get_db_connection()
        with conn:
            conn.execute("DELETE FROM credit_transactions WHERE user_id IN (%s, %s, %s)", (self.uid, self.other_uid, self.admin_uid))
            conn.execute("DELETE FROM credit_wallets WHERE user_id IN (%s, %s, %s)", (self.uid, self.other_uid, self.admin_uid))
            conn.execute("DELETE FROM assessment_attempts WHERE user_id IN (%s, %s, %s)", (self.uid, self.other_uid, self.admin_uid))
            conn.execute("DELETE FROM user_profiles WHERE user_id IN (%s, %s, %s)", (self.uid, self.other_uid, self.admin_uid))
            conn.execute("DELETE FROM users WHERE id IN (%s, %s, %s)", (self.uid, self.other_uid, self.admin_uid))
        conn.close()
    ...
    def test_case_04_open_unlocked_historical_assessment(self):
        ...
        conn = get_db_connection()
        with conn:
            conn.execute("UPDATE assessment_attempts SET is_unlocked = 1 WHERE id = %s", (aid,))
        conn.close()
    ...
    def test_case_05_open_locked_historical_assessment(self):
        ...
        # Validate UI flow sequence mapping (simulating the client behavior)
        # Attempting to fetch teaser without caching
        t_res = self.client.get(f"/api/assessment/{aid}/teaser")
        self.assertEqual(t_res.status_code, 200)
        t_data = t_res.get_json()
