...
class TestDeveloperDashboardSourceOfTruth(unittest.TestCase):
    ...
    def test_02_failed_and_cancelled_payment_contribute_zero(self):
        ...
        try:
            with conn:
                conn.execute("INSERT INTO users (id, email, password_hash, role, is_active) VALUES (%s, %s, 'hash', 'USER', 1)",
                ...
                conn.execute("""
                    ...
                    VALUES (%s, %s, 'plan_single', 19.0, 19.0, 'FAILED', CURRENT_TIMESTAMP)
                """, (f"pay_fail_{uuid.uuid4().hex[:8]}", test_user_id))

                # 2. Create a PENDING/CREATED payment of Rs.599
                conn.execute("""
                    INSERT INTO payments (id, user_id, plan_id, amount, original_amount, status, created_at)
                    VALUES (%s, %s, 'plan_pack30', 599.0, 599.0, 'CREATED', CURRENT_TIMESTAMP)
            ...
            with conn:
                conn.execute("DELETE FROM payments WHERE user_id = %s", (test_user_id,))
        ...
        try:
            with conn:
                ...
                conn.execute("""
                    ...
                    VALUES (%s, %s, 'plan_single', %s, 19.0, 3.8, 'SUCCESS', CURRENT_TIMESTAMP)
                ...
                conn.execute("""
                    ...
                    VALUES (%s, %s, 'PURCHASE', 1, 1, 'payment', %s, 'Purchased 1 credit pack', CURRENT_TIMESTAMP)
            ...
            with conn:
                conn.execute("DELETE FROM credit_transactions WHERE user_id = %s", (test_user_id,))
                conn.execute("DELETE FROM payments WHERE user_id = %s", (test_user_id,))
                conn.execute("DELETE FROM users WHERE id = %s", (test_user_id,))
            conn.close()
    ...
    def test_04_zero_rupee_100_percent_discount_redemption(self):
        ...
        try:
            with conn:
                ...
                conn.execute("""
                    INSERT INTO payments (id, user_id, plan_id, amount, original_amount, discount_amount, status, created_at)
                    VALUES (%s, %s, 'plan_single', 0.0, 19.0, 19.0, 'SUCCESS', CURRENT_TIMESTAMP)
                """, (payment_id, test_user_id))

                # Credit transaction with reference_type = 'referral_discount'
                conn.execute("""
                    INSERT INTO credit_transactions (id, user_id, type, amount, balance_after, reference_type, reference_id, description, created_at)
                    VALUES (%s, %s, 'PURCHASE', 1, 1, 'referral_discount', %s, 'Redeemed 100%% discount', CURRENT_TIMESTAMP)
                """, (f"tx_{uuid.uuid4().hex[:8]}", test_user_id, payment_id))
    ...
    def test_05_assessment_unlock_and_multi_attempt_flow(self):
        ...
        try:
            with conn:
                conn.execute("INSERT INTO users (id, email, password_hash, role, is_active) VALUES (%s, %s, 'hash', 'USER', 1)",
                             (test_user_id, f"{test_user_id}@test.ai"))
                conn.execute("INSERT INTO credit_wallets (id, user_id, balance, updated_at) VALUES (%s, %s, 1, CURRENT_TIMESTAMP)",
            ...
            with conn:
                conn.execute("UPDATE assessment_attempts SET is_unlocked = 1, unlocked_at = CURRENT_TIMESTAMP WHERE id = %s", (attempt_id,))
            ...
            with conn:
                conn.execute("DELETE FROM credit_transactions WHERE user_id = %s", (test_user_id,))
                conn.execute("DELETE FROM credit_wallets WHERE user_id = %s", (test_user_id,))
                conn.execute("DELETE FROM assessment_attempts WHERE user_id = %s", (test_user_id,))
                conn.execute("DELETE FROM users WHERE id = %s", (test_user_id,))
            conn.close()
    ...
    def test_06_wallet_ledger_100_percent_reconciled(self):
        ...
        try:
            ...
            cursor.execute("""
                ...
                HAVING COALESCE(w.balance, 0) - COALESCE(SUM(ct.amount), 0) != 0
            """)
    ...
    def test_07_authorization_and_security_checks(self):
        ...
        try:
            with conn:
                ...
                conn.execute("INSERT INTO users (id, email, password_hash, role, is_active) VALUES (%s, %s, 'hash', 'DEVELOPER', 0)",
            ...
            with conn:
                conn.execute("DELETE FROM users WHERE id IN (%s, %s)", (student_id, dev_id))
            conn.close()
    ...
    def test_08_assessment_engine_integrity(self):
        ...
        questions_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'questions', 'english.json'))
