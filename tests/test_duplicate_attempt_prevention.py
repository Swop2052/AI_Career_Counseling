...
class TestDuplicateAttemptPrevention:
    ...
    def test_concurrent_or_rapid_submissions_return_same_attempt(self):
        ...
        c = conn.cursor()
        c.execute("SELECT count(*) FROM assessment_attempts WHERE user_id = %s", (user_id,))
        count = c.fetchone()[0]
        conn.close()
        assert count == 1
        ...
        with conn:
            conn.execute("""
                INSERT INTO assessment_attempts (
                    id, user_id, student_profile, riasec_answers, riasec_scores, riasec_code,
                    teaser_data, full_result_data, is_unlocked, created_at, completed_at
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 1, '2026-09-05T18:47:08.200', '2026-09-05T18:47:08.200')
            """, (
                cluster_unlocked_id,
                user_id,
                json.dumps(SAMPLE_PROFILE),
            ...
            conn.execute("""
                INSERT INTO assessment_attempts (
                    id, user_id, student_profile, riasec_answers, riasec_scores, riasec_code,
                    teaser_data, full_result_data, is_unlocked, created_at, completed_at
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 0, '2026-09-05T18:47:08.300', '2026-09-05T18:47:08.300')
            """, (
                cluster_locked_id,
                user_id,
                json.dumps(SAMPLE_PROFILE),
