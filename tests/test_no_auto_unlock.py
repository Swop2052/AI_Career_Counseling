...
class TestNoAutoUnlockFlow(unittest.TestCase):
    ...
    def create_completed_locked_assessment(self, user_id):
        ...
        try:
            row = conn.execute("SELECT is_unlocked, unlocked_at FROM assessment_attempts WHERE id = %s", (attempt_id,)).fetchone()
    ...
    def test_scenario_b_explicit_unlock_assessment(self):
        ...
        conn = get_db_connection()
        try:
            row_before = conn.execute("SELECT is_unlocked FROM assessment_attempts WHERE id = %s", (attempt_id,)).fetchone()
    ...
    def test_scenario_d_free_code_redemption_no_auto_unlock(self):
        ...
        conn = get_db_connection()
        try:
            row = conn.execute("SELECT is_unlocked FROM assessment_attempts WHERE id = %s", (attempt_id,)).fetchone()
            self.assertEqual(row['is_unlocked'], 1)
        finally:
            conn.close()
    ...
    def test_scenario_e_multiple_locked_assessments_isolation(self):
        ...
        conn = get_db_connection()
        try:
            row_a = conn.execute("SELECT is_unlocked FROM assessment_attempts WHERE id = %s", (attempt_a,)).fetchone()
            row_b = conn.execute("SELECT is_unlocked FROM assessment_attempts WHERE id = %s", (attempt_b,)).fetchone()
