...
class TestRazorpayCheckoutLifecycle(unittest.TestCase):
    ...
    def test_01_pay_cancel_pay_again(self):
        ...
        p1 = conn.execute("SELECT * FROM payments WHERE razorpay_order_id = %s", (order_id_1,)).fetchone()
        conn.close()
        ...
        conn = get_db_connection()
        p1_cancelled = conn.execute("SELECT * FROM payments WHERE razorpay_order_id = %s", (order_id_1,)).fetchone()
        conn.close()
        ...
        conn = get_db_connection()
        p2 = conn.execute("SELECT * FROM payments WHERE razorpay_order_id = %s", (order_id_2,)).fetchone()
        conn.close()
    ...
    def test_02_multi_cancel_cycle_pay_cancel_pay_cancel_pay(self):
        ...
        conn = get_db_connection()
        s1 = conn.execute("SELECT status FROM payments WHERE razorpay_order_id = %s", (order_ids[0],)).fetchone()['status']
        s2 = conn.execute("SELECT status FROM payments WHERE razorpay_order_id = %s", (order_ids[1],)).fetchone()['status']
        s3 = conn.execute("SELECT status FROM payments WHERE razorpay_order_id = %s", (order_ids[2],)).fetchone()['status']
        conn.close()
    ...
    def test_03_successful_payment_after_cancel(self):
        ...
        conn = get_db_connection()
        pay_row = conn.execute("SELECT * FROM payments WHERE razorpay_order_id = %s", (success_order_id,)).fetchone()
        conn.close()
    ...
    def test_05_failed_signature_verification(self):
        ...
        conn = get_db_connection()
        row = conn.execute("SELECT status FROM payments WHERE razorpay_order_id = %s", (order_id,)).fetchone()
        conn.close()
