        ...
        const uiTranslations = {
            en: {
                ...
                "contact-title": "Get in <span class=\"grad\">Touch</span>",
            ...
            hi: {
                ...
                "contact-title": "संपर्क <span class=\"grad\">करें</span>",
            ...
            mr: {
                ...
                "contact-title": "संपर्क <span class=\"grad\">साधा</span>",
        ...
        function translateStaticUI(lang) {
            ...
            if (brandEl) {
                brandEl.innerHTML = `<img src="/static/logo.png" alt="SkillSense Logo" class="brand-logo" style="height: 34px; width: 34px; object-fit: contain; filter: drop-shadow(0 0 8px rgba(0, 212, 255, 0.4)); margin-right: 8px;"> SkillSense`;
            }
            ...
            if (contactEyebrow) contactEyebrow.textContent = dict['contact-eyebrow'] || 'Get in touch';
            
            const contactTitle = document.querySelector('#contact-section .sec-head h2');
            if (contactTitle) contactTitle.innerHTML = dict['contact-title'] || 'Get in <span class="grad">Touch</span>';
        ...
        function showLinkedInShareModal(data) {
            ...
            const postText = `🌟 My SkillSense Career Assessment Results! 🚀\n\n` +
                `I just completed the AI-powered career discovery assessment on SkillSense! Here is my profile:\n\n` +
                `👤 Name: ${studentName}\n` +
                `🧠 Key Traits: ${traitListText}\n` +
                `📊 Personality Code: ${profile.riasec_code || '---'} (Estimated MBTI: ${mbti})\n\n` +
                `🎯 Top Recommended Careers Mapped:\n${careersText}\n` +
                `Explore your own matched career paths and try the AI Career Counselor VERA!\n\n` +
                `#${studentHashtag} ${careerHashtags} #VitalsAndVectors #NewAICareerGuidance #SkillSense #CareerDiscovery #AI #CareerCounseling`;
            ...
            modalOverlay.innerHTML = `
                <div class="trait-modal glass" style="max-width: 550px; padding: 24px; border: 1px solid rgba(255,255,255,0.12);">
                    <div class="modal-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 16px;">
                        <h3 style="margin:0; font-size:1.4rem; display:flex; align-items:center; gap:8px;">
                            <span style="font-size:1.6rem">🚀</span> Share Your Success!
                        </h3>
                    ...
                    <p style="font-size:0.92rem; color:var(--muted); margin-bottom:16px; line-height:1.4; text-align:left;">
                        Congratulations, <b>${studentName}</b>! Copy the customized post template below and share your achievements with your network on LinkedIn.
                    </p>
                    
                    <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; padding: 16px; margin-bottom: 20px; max-height: 200px; overflow-y: auto; text-align:left;">
                        <pre id="sharePostText" style="white-space: pre-wrap; font-family: inherit; font-size: 0.9rem; color: var(--text); margin: 0; line-height:1.5;">${postText}</pre>
                    </div>
                    
                    <div style="display:flex; gap:12px; justify-content:flex-end;">
                        <button id="copyPostBtn" class="btn" style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); color: var(--text); padding: 10px 16px; font-weight: 500; font-size: 0.9rem; display: inline-flex; align-items: center; gap: 6px; cursor: pointer; border-radius: 6px; transition: all 0.2s;">
                            📂 Copy Post
                        </button>
                        <button id="linkedinGoBtn" class="btn btn-primary" style="padding: 10px 20px; font-weight: 600; font-size: 0.9rem; display: inline-flex; align-items: center; gap: 6px; cursor: pointer; border-radius: 6px; transition: all 0.2s;">
                            🔗 Share on LinkedIn
                        </button>
                    </div>
                </div>
        ...
        function showDownloadFeedbackModal(career) {
            ...
            overlay.style.zIndex = '999999'; // Ensure it sits on top of all modal overlays
        ...
        function downloadCareerPDF(career) {
            ...
            if (!careerFeedbackSubmitted[careerName]) {
            ...
            const renderStudySection = (filterActive) => {
                ...
                return `
                    <div class="grid-3" id="modalInstitutesGrid">
                        ${showGov ? `
                        <div>
                            <h5 style="color:var(--amber);margin-bottom:8px;font-size:0.85rem;display:flex;align-items:center;gap:4px;">🏛️ ${dict['title-gov'] || 'Government'} ${prefRange === 'government' ? '<span style="color:var(--cyan);font-size:0.75rem;">(Preferred)</span>' : ''}</h5>
                            ${govListHtml}
                        </div>
                        ` : ''}
                        ${showPriv ? `
