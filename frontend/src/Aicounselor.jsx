...
export default function AICounselor({ currentUser, onLoginRequest }) {
  const [chatOpen, setChatOpen] = useState(false);
  const [messages, setMessages] = useState([
    {
      id: 1,
      sender: 'bot',
      text: "Hi there! 👋 I'm your AI Career Counselor. Ask me anything about subjects, careers, or your quiz results!"
    }
  ]);
  ...
  const handleSend = async (text) => {
    ...
    if (!currentUser?.email) {
      ...
      setMessages((prev) => [
        ...prev, 
        newMsg,
        { id: Date.now() + 1, sender: 'bot', text: "First login ok" }
      ]);
      setInputValue('');
      setTimeout(() => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ message: text, email: currentUser.email })
      });
  ...
  return (
    <div className="fixed bottom-5 right-5 md:bottom-7 md:right-7 z-50 flex flex-col items-end gap-3 pointer-events-none">
      ...
      >
        ...
        <div className="p-4 bg-[#CFEDED]/20 flex-1 overflow-y-auto space-y-3 min-h-[120px]">
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`text-sm p-3 shadow-sm max-w-[85%] ${
                m.sender === 'user' 
                  ? 'bg-[#09A3A3] text-white rounded-2xl rounded-tr-sm' 
                  : 'bg-white text-[#0B3D3D] rounded-2xl rounded-tl-sm'
              }`}>
                {m.text}
              </div>
            </div>
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-white rounded-2xl rounded-tl-sm p-3 text-sm text-[#0B3D3D] shadow-sm flex items-center gap-1">
                <span className="w-1.5 h-1.5 bg-[#09A3A3]/40 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                <span className="w-1.5 h-1.5 bg-[#09A3A3]/60 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                <span className="w-1.5 h-1.5 bg-[#09A3A3]/80 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
              </div>
            </div>
          )}
        ...
        <div className="flex items-center gap-2 p-3 border-t border-black/5 bg-white shrink-0">
          <input
            type="text"
            value={inputValue}
          ...
          <button 
            ...
            className="rounded-full bg-gradient-to-br from-[#09A3A3] to-[#04302E] text-white p-2.5 shadow-md shadow-[#09A3A3]/30 hover:brightness-110 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
